// Slack-style Activity view for the 옆커폰 fork: the data side.
//
// Slack's Activity page lists mentions, thread replies, reactions and
// direct messages in one feed with filter tabs. This module keeps the
// view's state (visible, active tab), fetches each feed (GET /messages
// with the matching narrow, and our own endpoint for thread replies),
// merges them, and builds the row contexts. The DOM side (the
// container in the middle column, the tabs, the show/hide protocol
// shared with the inbox and recent views) lives in
// ykphone_activity_ui.ts.
...
export type ActivityTabContext = {
    id: ActivityTab;
    label: string;
    active: boolean;
};
...
let visible = false;
let active_tab: ActivityTab = "all";
// Responses of an earlier load (another tab, or a re-shown view) are
...
export function get_active_tab(): ActivityTab {
    return active_tab;
}

export function set_active_tab(tab: ActivityTab): void {
    active_tab = tab;
}
...
export function tabs_context(): ActivityTabContext[] {
    return TABS.map((tab) => ({id: tab, label: tab_label(tab), active: tab === active_tab}));
}
