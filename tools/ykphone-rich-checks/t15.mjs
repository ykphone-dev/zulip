...
await sleep(500);
console.log("drafts", await page.evaluate(() => { const k = Object.keys(localStorage).find((k) => k.includes("drafts")); return k ? Object.values(JSON.parse(localStorage.getItem(k))).map((d) => d.content) : null; }));
...
await page.evaluate(() => { location.hash = "#drafts"; }); await sleep(2500);
await shot(page, "t15-drafts");
console.log("overlay rows", await page.$$eval(".draft-row, .draft-message-row, [data-draft-id]", (els) => els.slice(0, 3).map((e) => e.textContent.trim().replace(/\s+/g, " ").slice(0, 60))));
const row = await page.$("[data-draft-id] .restore-overlay-message.message_content, [data-draft-id] .messagebox");
