...
const drafts = () => page.evaluate(() => Object.values(JSON.parse(localStorage.getItem("drafts") ?? "{}")).map((d) => d.content));
await page.goto(BASE + "/#narrow/channel/12-errors/topic/general.20chat");
await sleep(3500);
await page.click(".ykphone-rich-content");
await kb.type("draft **bold** and @iag"); await sleep(400); await kb.press("Enter"); await sleep(200);
await kb.type("done");
console.log("typed", JSON.stringify((await state(page)).md));
await page.goto(BASE + "/#narrow/channel/11-devel/topic/general.20chat").catch(() => {});
await sleep(2500);
...
await page.goto(BASE + "/#drafts"); await sleep(2000);
console.log("draft rows", await page.$$eval(".draft-row .rendered_markdown, .draft-info-box .messagebox-content", (els) => els.slice(0,3).map((e) => e.textContent.trim().slice(0, 60))));
await browser.close();
