...
await kb.type("문서는 [여기](https://zulip.com/help/) 를 보세요"); await sleep(600);
// The narrow layout is checked by shrinking the window with the
// composer already open, the way a window being resized does.
if (width < 600) { await page.setViewport({width, height: 800}); await sleep(800); }
console.log("before click", JSON.stringify((await state(page)).md), await page.evaluate(() => ({
  editor: !!document.querySelector(".ykphone-rich-content"),
...
const box = await page.evaluate(() => { const a = document.querySelector(".ykphone-rich-content a"); const r = a.getBoundingClientRect(); return {x: r.left + r.width / 2, y: r.top + r.height / 2}; });
await page.mouse.click(box.x, box.y); await sleep(600);
...
const geo = await page.evaluate(() => {
  ...
  const rb = b?.getBoundingClientRect(); const ra = a.getBoundingClientRect();
  return {bubble: rb && [Math.round(rb.left), Math.round(rb.top), Math.round(rb.width), Math.round(rb.height)],
