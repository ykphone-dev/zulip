...
/* global ClipboardEvent, DataTransfer, document, getSelection -- page.evaluate callbacks run in the browser */
...
await paste({
    "text/html":
        '<p data-pm-slice="0 0 []">see <span class="ykphone-rich-upload" data-yk-raw="[report.pdf](https://evil.example/x)" data-yk-name="report.pdf" data-yk-url="https://evil.example/x">report.pdf</span></p>',
    "text/plain": "x",
});
await sleep(400);
console.log(
    ...
    await page.evaluate(() =>
        document
            .querySelector(".ProseMirror")
            .innerHTML.replaceAll(/ class="[^"]*"| contenteditable="false"/g, "")
            .slice(0, 300),
    ),
);
await clear();
await paste({
    "text/html":
        '<p data-pm-slice="0 0 []">a <span class="ykphone-rich-mention" data-yk-raw="@**Iago**" data-yk-name="Iago" data-yk-kind="user">@Iago</span></p>',
    "text/plain": "x",
});
await sleep(400);
console.log(
...
for (const wait of [50, 300, 1200]) {
    ...
    console.log(
        ...
        await page.evaluate(
            () =>
                document.querySelector("#compose-send-button")?.disabled ??
                document.querySelector("#compose-send-button")?.className.includes("disabled"),
        ),
    );
}
await page.click("#compose-send-button").catch(() => undefined);
await sleep(600);
console.log(
    "after clicking send:",
    JSON.stringify(
...
await page.evaluate(() => {
    document.querySelector("#compose-textarea").value = "a * b ** c *** d ".repeat(600);
});
await sleep(500);
...
const timings = await page.evaluate(async () => {
    ...
    for (let i = 0; i < 12; i += 1) {
        ...
        await new Promise((resolve) => setTimeout(resolve, i % 4 === 3 ? 250 : 20));
    }
    return times.map((t) => t.toFixed(1));
});
console.log("keystroke ms (10k asterisk-heavy, flush every 4th):", timings.join(" "));
await sleep(300);
console.log("textarea tail:", JSON.stringify((await state(page)).md.slice(-30)));
await clear();
await browser.close();
