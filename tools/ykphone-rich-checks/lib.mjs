...
export async function start({width = 1280, height = 900, user = "iago@zulip.com"} = {}) {
    const browser = await puppeteer.launch({
        executablePath: "/home/vagrant/.cache/ms-playwright/chromium-1243/chrome-linux-arm64/chrome",
        headless: true,
    ...
    page.on("console", (msg) => {
        if ((["error", "warning"].includes(msg.type()) || msg.text().startsWith("YKDEBUG")) && !msg.text().includes("ws://") && !msg.text().includes("webpack-dev-server")) {
            console.log("CONSOLE", msg.type(), msg.text());
        }
    });
    await page.goto(BASE + "/devlogin/");
    await page.waitForSelector(`input[value="${user}"], button[value="${user}"]`);
    await Promise.all([page.waitForNavigation({waitUntil: "networkidle2"}), page.click(`[value="${user}"]`)]);
