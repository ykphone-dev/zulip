...
const rows = await page.$$(
    ".draft-message-row .restore-overlay-message.message_content, .draft-row .restore-overlay-message.message_content",
);
console.log(
    "draft rows",
    rows.length,
    await page.$$eval(".restore-overlay-message.message_content", (els) =>
        els.slice(0, 3).map((e) => e.textContent.trim().slice(0, 50)),
    ),
);
if (rows.length > 0) {
    await rows[0].click();
