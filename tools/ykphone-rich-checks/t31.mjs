    ...
    page.evaluate(() => ({
        ...
        focus:
            document.activeElement === document.body
                ? "BODY"
                : document.activeElement?.className?.toString().slice(0, 34),
        caret: (() => {
...
await sleep(200);
await ctrl_k();
await snap("ctrl+k, no selection");
await kb.type("https://www.yeopkerphone.co.kr");
await kb.press("Enter");
await sleep(500);
await snap("enter with the URL in the text field");
await kb.type("다음");
await sleep(250);
await snap("typing after");
// Enter with text that is not an address and an empty link field.
await clear();
await kb.type("a ");
await ctrl_k();
await kb.type("just words");
await kb.press("Enter");
await sleep(400);
await snap("enter with words, empty link field");
await kb.type("ex.co.kr");
await kb.press("Enter");
await sleep(500);
await snap("then a bare domain in the link field");
await kb.type("!");
await sleep(200);
await snap("typing after");
// Escape.
...
await kb.type("b ");
await ctrl_k();
await kb.type("https://ex.com");
await kb.press("Escape");
await sleep(400);
await snap("escape");
await kb.type("c");
await sleep(200);
await snap("typing after escape");
...
await kb.up("Shift");
await ctrl_k();
await snap("ctrl+k over a selection");
await kb.type("zulip.com");
await kb.press("Enter");
await sleep(500);
await snap("enter over a selection");
await kb.type(".");
await sleep(200);
await snap("typing after");
await browser.close();
